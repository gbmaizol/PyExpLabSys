...
class PowerWalkerEthernet(object):
    ...
    def _read_static_data(self):
        ...
        nominal_input = int(lines[5][0:3])
        nominal_output = int(lines[5][4:])
        values = {
            'model': lines[3],
            'version': lines[7],
            'nominal_input_voltage': nominal_input,
            'nominal_output_voltage': nominal_output,
            'nominal_output_frequency': int(lines[11]) / 10.0,
            'rated_battery_voltage': int(lines[13]) / 10.0,
            'rated_va': int(lines[9]),
            'rated_output_current': int(lines[12]) / 10.0
        }
        return values
    ...
    def device_status(self):
        ...
        status = []   # TODO!
        if not lines[2] == 'Line Mode':
            status.append('Utility Fail')  # Compatibility with serial interface

        values = {
            'input_voltage': int(lines[15]) / 10.0,
            'output_voltage': int(lines[18]) / 10.0,
            'output_current': int(lines[38]) / 10.0,
            'input_frequency': int(lines[14]) / 10.0,
            'battery_voltage': int(lines[11]) / 10.0,
            'temperature': int(lines[3]) / 10.0,
            'status': status,
            'battery_capacity': int(lines[12]),
            'remaining_battery': lines[13],  # minutes
            'output_frequency': int(lines[17]) / 10.0,
            'load_level': int(lines[20])
        }
